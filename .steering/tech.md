...
- **Compute & API:** Modal.com (serverless). Daily ingest via `modal.Cron`; FastAPI served via `@modal.asgi_app` with scale-to-zero.
- **Database:** Neon or Supabase — free tier with PostGIS.
...
- Compute: serverless (Modal or equivalent), scale-to-zero between runs.
- Database: free-tier Postgres with PostGIS.
